...
| --- | --- | --- | --- |
| R1 | Repo-managed config and Helm templates can accidentally receive real secrets | High | Use the `redact-config` clean filter plus pre-commit hook for `configs/config.yaml`; review Helm/runtime config before committing |
...
- `configs/config.yaml` can contain real local values in the worktree, but Git must receive only the sanitized blob generated by `scripts/git/redact-config-yaml.sh`.
