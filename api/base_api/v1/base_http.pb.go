...
const OperationBaseGetPermCode = "/api.base_api.v1.Base/GetPermCode"
...
func RegisterBaseHTTPServer(s *http.Server, srv BaseHTTPServer) {
	r := s.Route("/")
	r.POST("/basic-api/login", _Base_Login0_HTTP_Handler(srv))
	r.GET("/basic-api/getUserInfo", _Base_GetUserInfo0_HTTP_Handler(srv))
	r.GET("/basic-api/getPermCode", _Base_GetPermCode0_HTTP_Handler(srv))
	r.GET("/basic-api/logout", _Base_Logout0_HTTP_Handler(srv))
	r.GET("/basic-api/getMenuList", _Base_GetMenuList0_HTTP_Handler(srv))
...
func _Base_GetPermCode0_HTTP_Handler(srv BaseHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationBaseGetPermCode)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetPermCode(ctx, req.(*emptypb.Empty))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*GetPermCodeReply)
		return ctx.Result(200, reply.PermCodeList)
	}
}
...
type BaseHTTPClient interface {
	...
	GetMenuList(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *GetMenuListReply, err error)
	GetPermCode(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *GetPermCodeReply, err error)
...
func (c *BaseHTTPClientImpl) GetMenuList(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*GetMenuListReply, error) {
	...
	pattern := "/basic-api/getMenuList"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationBaseGetMenuList))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out.MenuList, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
...
func (c *BaseHTTPClientImpl) GetPermCode(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*GetPermCodeReply, error) {
	var out GetPermCodeReply
	pattern := "/basic-api/getPermCode"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationBaseGetPermCode))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out.PermCodeList, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
