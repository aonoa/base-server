...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withDept; query != nil {
		if err := uq.loadDept(ctx, query, nodes,
			func(n *User) { n.Edges.Dept = []*Dept{} },
			func(n *User, e *Dept) { n.Edges.Dept = append(n.Edges.Dept, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadDept(ctx context.Context, query *DeptQuery, nodes []*User, init func(*User), assign func(*User, *Dept)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.DeptTable)
		s.Join(joinT).On(s.C(dept.FieldID), joinT.C(user.DeptPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.DeptPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.DeptPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "dept" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
